...
cleanmetadata_file() {
  ...
  if [[ "$replace_original" == "1" ]]; then
    ...
    local backup_path="${f}.bak"
    if ! mv "$f" "$backup_path"; then
        error "Failed to create backup of original file!"
        return 1
    fi

    if mv "$source_to_move" "$f"; then
        if command -v shred &>/dev/null; then
            shred -u -n 1 "$backup_path" 2>/dev/null || rm -f "$backup_path"
        else
            rm -f "$backup_path"
        fi
        print_operation_end "Original file replaced"
    else
        error "Failed to move processed file! Restoring backup..."
        mv "$backup_path" "$f" 2>/dev/null || error "CRITICAL: Backup at $backup_path could not be restored!"
        return 1
    fi
  else
